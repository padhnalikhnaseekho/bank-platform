...
final class AccountMapper {
    ...
    /**
     * Mutates a managed entity in place (rather than building a fresh detached one) so Hibernate's
     * optimistic-lock check on {@code @Version} always compares against the row it just loaded in
     * this transaction, not a stale/ambiguous reconstructed value.
     */
    static void applyToExisting(AccountEntity entity, Account account) {
        entity.applyMutableState(
                account.status().name(), account.balance().amount(), account.updatedAt());
    }
