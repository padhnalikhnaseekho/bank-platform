...
class ApplyTransferUseCaseTest {
    ...
    void debitsSourceAndCreditsTargetOnSuccessfulTransfer() {
        Account source =
                Account.open(UUID.randomUUID(), "111111111111", AccountType.SAVINGS, "INR");
        Account target =
                Account.open(UUID.randomUUID(), "222222222222", AccountType.SAVINGS, "INR");
        ...
        verify(eventPublisher)
                .publish(eq("transfer-completed"), eq("Transfer"), eq(transactionId), any());
    }
    ...
    void publishesTransferFailedWhenSourceHasInsufficientFunds() {
        ...
        verify(accountRepository, never()).save(any());
        verify(eventPublisher)
                .publish(eq("transfer-failed"), eq("Transfer"), eq(transactionId), any());
    }
}
