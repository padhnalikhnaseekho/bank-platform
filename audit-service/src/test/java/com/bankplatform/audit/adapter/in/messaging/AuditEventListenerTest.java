...
class AuditEventListenerTest {
    ...
    @BeforeEach
    void setUp() {
        listener = new AuditEventListener(recordAuditEventUseCase, idempotentEventProcessor, objectMapper);
    }
    ...
    void recordsEveryIncomingEventRegardlessOfType() {
        ...
        EventEnvelope envelope = new EventEnvelope(eventId, "account-created", 1, Instant.now(), "account-service",
                "corr-1", null, "Account", "acc-1", "acc-1", new SamplePayload("value"));
        ...
        verify(recordAuditEventUseCase).record(envelopeCaptor.capture(), payloadJsonCaptor.capture());
        org.assertj.core.api.Assertions.assertThat(envelopeCaptor.getValue().eventId()).isEqualTo(eventId);
        org.assertj.core.api.Assertions.assertThat(payloadJsonCaptor.getValue()).contains("\"field\":\"value\"");
    }

    @Test
    void skipsAlreadyProcessedEvents() {
        EventEnvelope envelope = new EventEnvelope(UUID.randomUUID(), "account-created", 1, Instant.now(),
                "account-service", "corr-1", null, "Account", "acc-1", "acc-1", new SamplePayload("value"));
        String message = objectMapper.writeValueAsString(envelope);
