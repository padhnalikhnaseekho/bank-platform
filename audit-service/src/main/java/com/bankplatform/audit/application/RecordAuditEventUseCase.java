...
public class RecordAuditEventUseCase {
    ...
    public void record(EventEnvelope envelope, String payloadJson) {
        AuditEvent event = AuditEvent.capture(envelope.eventId(), envelope.eventType(), envelope.aggregateType(),
                envelope.aggregateId(), payloadJson, envelope.correlationId(), envelope.occurredAt());
