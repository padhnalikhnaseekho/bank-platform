...
class PaymentFlowIntegrationTest extends PostgresTestcontainerBase {
    ...
    void createsAScheduledPaymentAndCancelsIt() throws Exception {
        ...
        String response = mockMvc
                .perform(post("/api/v1/payments/scheduled").contentType(MediaType.APPLICATION_JSON)
                        .content(body)
    ...
    void createsARecurringPayment() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/payments/recurring").contentType(MediaType.APPLICATION_JSON)
                        .content(body)
                        .with(jwt().jwt(j -> j.subject(customerId.toString()))
                                .authorities(new SimpleGrantedAuthority("ROLE_CUSTOMER"))))
                .andExpect(status().isCreated())
    ...
    void rejectsSchedulingAPaymentToTheSameAccount() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/payments/scheduled").contentType(MediaType.APPLICATION_JSON)
                        .content(body)
    ...
    void cancellingAnUnknownPaymentIsNotFound() throws Exception {
        mockMvc.perform(post("/api/v1/payments/" + UUID.randomUUID() + "/cancel")
                        .with(jwt().jwt(j -> j.subject(UUID.randomUUID().toString()))
                                .authorities(new SimpleGrantedAuthority("ROLE_CUSTOMER"))))
    ...
    void createScheduledPaymentWithoutTokenIsUnauthorized() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/payments/scheduled").contentType(MediaType.APPLICATION_JSON).content(body))
