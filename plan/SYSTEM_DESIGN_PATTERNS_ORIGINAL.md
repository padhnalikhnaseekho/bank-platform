> **Note:** this is the original version of `SYSTEM_DESIGN_PATTERNS.md` as first committed
> (`1537991`), kept here for reference rather than deleted. Several of its claims turned out to
> be inaccurate against the actual codebase — most notably that the outbox pattern, idempotency
> keys, and Kafka Streams were "missing" or "stubbed" (all three are fully implemented), and
> that a circuit breaker was needed because of a synchronous Account↔Transaction HTTP call that
> doesn't exist anywhere in this codebase. See `SYSTEM_DESIGN_PATTERNS.md` for the corrected
> version and `docs/adr/` for the reasoning behind the actual design decisions it references.
