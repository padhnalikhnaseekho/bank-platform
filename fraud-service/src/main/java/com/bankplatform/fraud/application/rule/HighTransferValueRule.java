...
/**
 * Flags a customer whose total outgoing transfer value exceeds {@code maxTransferAmount} within the
 * window.
 */
@Component
public class HighTransferValueRule implements FraudRule {
    ...
    public HighTransferValueRule(
            @Value("${bank-platform.fraud.rules.max-transfer-amount:50000}")
                    BigDecimal maxTransferAmount) {
        this.maxTransferAmount = maxTransferAmount;
    }

    @Override
    public Optional<FraudAlert> evaluate(
            String customerId, TransferWindowStats stats, Instant windowStart, Instant windowEnd) {
        ...
        return Optional.of(
                FraudAlert.of(
                        customerId,
                        FraudAlertType.HIGH_TRANSFER_VALUE,
                        stats,
                        windowStart,
                        windowEnd,
                        message));
