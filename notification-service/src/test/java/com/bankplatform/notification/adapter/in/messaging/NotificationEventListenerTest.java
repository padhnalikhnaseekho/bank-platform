...
class NotificationEventListenerTest {
    ...
    @BeforeEach
    void setUp() {
        listener = new NotificationEventListener(sendNotificationUseCase, idempotentEventProcessor, objectMapper);
        doAnswer(invocation -> {
    ...
    private record UserCreatedPayload(String userId, String email, String fullName, List<String> roles) {}
    ...
    void userCreatedSendsAWelcomeEmail() {
        ...
        String message = toMessage("user-created",
                new UserCreatedPayload(userId.toString(), "alice@example.com", "Alice", List.of("CUSTOMER")));

        listener.onUserCreated(message);

        verify(sendNotificationUseCase).send(eq(userId), eq(Channel.EMAIL), eq("welcome"),
    ...
    void moneyDepositedNotifiesTheAccountOwner() {
        ...
        String message = toMessage("money-deposited", new MoneyMovementPayload(UUID.randomUUID().toString(),
                UUID.randomUUID().toString(), customerId.toString(), new BigDecimal("50.00"), "INR", "COMPLETED",
                null));

        listener.onMoneyDeposited(message);

        verify(sendNotificationUseCase).send(eq(customerId), eq(Channel.EMAIL), eq("deposit-confirmation"),
                anyString());
    }
    ...
    void moneyWithdrawnNotifiesTheAccountOwner() {
        UUID customerId = UUID.randomUUID();
        String message = toMessage("money-withdrawn", new MoneyMovementPayload(UUID.randomUUID().toString(),
                UUID.randomUUID().toString(), customerId.toString(), new BigDecimal("20.00"), "INR", "COMPLETED",
                null));

        listener.onMoneyWithdrawn(message);

        verify(sendNotificationUseCase).send(eq(customerId), eq(Channel.EMAIL), eq("withdrawal-confirmation"),
                anyString());
    }
    ...
    void moneyMovementWithNoCustomerIdIsSkipped() {
        String message = toMessage("money-deposited", new MoneyMovementPayload(UUID.randomUUID().toString(),
                UUID.randomUUID().toString(), null, new BigDecimal("50.00"), "INR", "FAILED", "Account not found"));
    ...
    void transferCompletedNotifiesBothSourceAndTargetCustomers() {
        ...
        verify(sendNotificationUseCase).send(eq(sourceCustomerId), eq(Channel.EMAIL), eq("transfer-confirmation"),
                anyString());
        verify(sendNotificationUseCase).send(eq(targetCustomerId), eq(Channel.EMAIL), eq("transfer-confirmation"),
                anyString());
    }
    ...
    void transferFailedOnlyNotifiesTheSourceWhenTargetAccountWasMissing() {
        ...
        String message = toMessage("transfer-failed",
                new TransferPayload(UUID.randomUUID().toString(), UUID.randomUUID().toString(),
                        UUID.randomUUID().toString(), sourceCustomerId.toString(), null, new BigDecimal("40.00"),
    ...
    void notificationRequestedDispatchesToTheRequestedChannel() {
        ...
        String message = toMessage("notification-requested",
                new NotificationRequestedPayload(recipientId.toString(), "PUSH", "custom-template", "custom body"));

        listener.onNotificationRequested(message);

        verify(sendNotificationUseCase).send(recipientId, Channel.PUSH, "custom-template", "custom body");
    }
}
